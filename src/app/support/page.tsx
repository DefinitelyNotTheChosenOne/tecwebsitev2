    ...
    <div className="h-screen bg-[#0f172a] text-white font-sans py-4 px-12 overflow-hidden flex flex-col items-center">
      <div className="w-full max-w-[1900px] h-full flex flex-col">
        ...
        <header className="flex items-center justify-between mb-4 px-10 shrink-0">
          <Link href={profile?.role === 'admin' ? "/admin/dashboard" : "/dashboard"} className="flex items-center gap-3 text-brand-secondary hover:text-white transition-all group">
            ...
            {profile?.role === 'admin' && !selectedUserId ? (
              ...
              <>
                <div className="p-10 border-b border-white/10 flex items-center gap-6 bg-white/[0.02]">
                  ...
                  <div>
                    <h1 className="text-2xl font-black tracking-tight leading-none mb-2 uppercase">Customer <span className="text-brand-primary italic">Support</span></h1>
                    <p className="text-brand-secondary text-[10px] font-black uppercase tracking-widest opacity-60">
                      {profile?.role === 'admin' ? `Secure Tunnel to Authorized User: ${selectedUserId?.substring(0,8)}` : 'Direct Tunnel to Marketplace Administration'}
                    </p>
                  </div>
                </div>
